...
interface SpitzingseeCacheEntry {
  raw: number      // as fetched from source
  jittered: number // raw + dateJitter, fixed at write time
}

interface SpitzingseeCache {
  lastUpdated: string
  temps: Record<string, SpitzingseeCacheEntry> // "DD.MM.YYYY 00:00" → { raw, jittered }
}
...
export async function fetchSpitzingseeTemperature(url: string): Promise<{
  ...
  try {
    ...
    // Use full ISO timestamp for cache keys
    const toDateKey = (d: Date) => d.toISOString()
    ...
    const merged: Record<string, SpitzingseeCacheEntry> = { ...(cache?.temps ?? {}) }
    ...
    // Support ISO (2024-03-06T00:00:00.000Z) and legacy German (06.03.2024 00:00) formats
    const allPoints = Object.entries(merged)
      .map(([date, entry]) => {
        ...
        if (date.includes("T") || date.match(/^\d{4}-\d{2}-\d{2}/)) {
          ...
          // Legacy German format: DD.MM.YYYY HH:MM
